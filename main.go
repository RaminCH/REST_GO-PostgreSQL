...
//SelectByID func
func SelectByID(id int) []Article {
	db, err := sql.Open("postgres", ConnStr)
	if err != nil {
		log.Fatal(err)
	}
...
func GetByID(w http.ResponseWriter, r *http.Request) {
	...
	idSel := SelectByID(id)
	json.NewEncoder(w).Encode(idSel)
}
...
func UpdateArticle(article Article, id int) {
	...
	_, err = db.Exec("update Articles set title = 'Updated title' where id=$1",
...
func main() {
	...
	//GetById/articles
	router.HandleFunc("/articles", GetByID).Methods("GET")

	//POST /article + json
	router.Handle("/article", jwtMiddleware.Handler(http.HandlerFunc(PostArticle))).Methods("POST")

	//PUT /article
	router.Handle("/article", jwtMiddleware.Handler(http.HandlerFunc(PutArticle))).Methods("PUT")
